...
describe('AuthService', function () {
  ...
  describe('LoginUser', () => {
    ...
    it('Should be Login Successful', async () => {
      jest.spyOn(userServiceMock, 'findUserByEmail').mockResolvedValueOnce({
        password: 'argon-hash-pass',
        name: 'test-name',
        id: 'object-id',
        role: UserRole.USER,
        email: 'test-mail',
      ...
      jest.spyOn(mockTokenService, 'generateToken').mockResolvedValueOnce({
  ...
  describe('Generate Token', () => {
    it('Should Be Throw BadRequest Because JWT is expired.', async () => {
      jest
        .spyOn(mockTokenService, 'verifyRefreshToken')
        .mockRejectedValueOnce(new JsonWebTokenError('error'));
    ...
    it('Should be Throw BadRequest Because Token Is Invalid', async () => {
      ...
      try {
        ...
        expect(mockTokenService.isValidate).toHaveBeenCalledWith(
          'test-tokenId',
          'invalid-token',
        );
      }
    ...
    it('Should be Throw BadRequest Because User Not found', async () => {
      ...
      try {
        ...
        expect(mockTokenService.invalidate).toHaveBeenCalledWith(
          'test-tokenId',
        );
    ...
    it('Should be Generate new Token Successfully', async () => {
      ...
      jest.spyOn(mockTokenService, 'invalidate').mockResolvedValueOnce();
      jest
        .spyOn(userServiceMock, 'findUserById')
        .mockResolvedValueOnce(fakeUserInput);
      jest.spyOn(mockTokenService, 'generateToken').mockResolvedValueOnce({
        accessToken: 'test-access',
